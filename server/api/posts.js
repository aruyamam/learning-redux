...
module.exports = function postsAPI(app) {
   ...
   app.post('/api/posts', expressJwt, (req, res) => {
      ...
      const { sub } = req.user;
      if (!sub) {
         throw new APIError('invalid token, you need to be logged in to create a post!');
      }
   ...
   app.post('/api/posts/:id', expressJwt, (req, res) => {
      const { sub } = req.user;
      if (!sub) {
         throw new Error('invvalid token, you need to be logged in to edit a post!');
      }
      ...
      if (currentPost !== sub) {
         return res.status(403).send({
            error: `user "${sub}" is not allowed to change post "${req.params.id}" (owner: "${
               currentPost.user
            }")`,
         });
      }
